...
void callpath_decrease_count(Callpath path)
{
  ...
  if (it_callpath2count != callpath2count.end()) {
    ...
    cerr << "mpileaks: ERROR: Found a path in handle2stack map, " <<
      "but no count found in callpath2count map" << endl;
  }
}


/* Todo on Monday: All of these functions are not handle-specific
 * so they should go into mpileaks.cpp (this file)...
 * Also, need to build and test the examples from Adam's directory... 
 * Adam's reduction code here: 
 * /usr/global/tools/mpileaks/src.reduction/mpileaks.cpp
